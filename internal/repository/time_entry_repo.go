...
func (r *TimeEntryRepo) Create(ctx context.Context, e *models.TimeEntry) error {
	...
	// that must not forget. Built by models.OpenKey so the writer and the
	// reader below cannot disagree about its shape.
	e.OpenKey = models.OpenKey(e.TenantID, e.EmployeeID)
	e.CreatedAt = now
	e.UpdatedAt = now
...
func (r *TimeEntryRepo) FindOpen(ctx context.Context, tenantID, employeeID primitive.ObjectID) (*models.TimeEntry, error) {
	var e models.TimeEntry
	err := r.col.FindOne(ctx, scoped(tenantID, bson.M{"open_key": models.OpenKey(tenantID, employeeID)})).Decode(&e)
	if err != nil {
...
func (r *TimeEntryRepo) Close(ctx context.Context, tenantID, id primitive.ObjectID, at time.Time, point models.GeoPoint, distanceM float64, workedMinutes int) error {
	res, err := r.col.UpdateOne(ctx,
		...
		scoped(tenantID, bson.M{"_id": id, "open_key": bson.M{"$exists": true}}),
...
func (r *TimeEntryRepo) List(ctx context.Context, tenantID primitive.ObjectID, q TimeEntryQuery) ([]*models.TimeEntry, error) {
	...
	cur, err := r.col.Find(ctx, scoped(tenantID, filter), options.Find().SetSort(bson.D{{Key: "clock_in_at", Value: 1}}))
	if err != nil {
		return nil, translate(err)
	}
	defer cur.Close(ctx)
